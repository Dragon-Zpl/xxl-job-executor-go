...
func (e *executor) RegScriptTask(glueType GlueType, task TaskFunc) {
	var t = &Task{}
	t.fn = e.chain(task)
...
func (e *executor) runTask(writer http.ResponseWriter, request *http.Request) {
	...
	if e.runList.Exists(Int64ToStr(param.JobID)) {
		if param.ExecutorBlockStrategy == coverEarly { //覆盖之前调度
			...
			e.log.WithContext(cxt).Error("任务[" + Int64ToStr(param.JobID) + "]已经在运行了:" + param.ExecutorHandler)
			return
		}
	}
	...
	if param.GlueType == GlueType_GLUE_GO.String() {
		...
		if !e.scriptList.Exists(param.GlueType) {
			...
			_, _ = writer.Write(returnCall(param, FailureCode, "GlueType not registered"))
			return
		}
		task = e.scriptList.Get(param.GlueType)
		task.Name = param.GlueType
	}
...
func (e *executor) registry() {
	...
	if err != nil {
		e.log.Fatal("执行器注册信息解析失败:" + err.Error())
	}
	for {
		<-t.C
